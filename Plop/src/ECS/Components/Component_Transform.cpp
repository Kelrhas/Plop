	...
	void Component_Transform::EditorUI()
	{
		...
		// Position
		{
			...
			if (ImGui::IsItemDeactivatedAfterEdit())
			{
				...
				UndoManager::RegisterAction(UndoAction::EntityMove(enttID, vBackup, vPosition));
			}
			...
			if (ImGui::SmallButton("X##Pos"))
			{
				...
				entt::entity enttID = entt::to_entity(Application::Get()->GetCurrentLevel().lock()->GetEntityRegistry(), *this);
				UndoManager::RegisterAction(UndoAction::EntityMove(enttID, vPosition, vNew));
		...
		// Rotation
		{
			...
			{
				entt::entity enttID = entt::to_entity(Application::Get()->GetCurrentLevel().lock()->GetEntityRegistry(), *this);
				UndoManager::RegisterAction(UndoAction::EntityRotate(enttID, qBackup, qRotation));
			}
			...
			if (ImGui::SmallButton("X##Rot"))
			{
				...
				entt::entity enttID = entt::to_entity(Application::Get()->GetCurrentLevel().lock()->GetEntityRegistry(), *this);
				UndoManager::RegisterAction(UndoAction::EntityRotate(enttID, qRotation, qNew));
		...
		// Scale
		{
			...
			{
				entt::entity enttID = entt::to_entity(Application::Get()->GetCurrentLevel().lock()->GetEntityRegistry(), *this);
				UndoManager::RegisterAction(UndoAction::EntityScale(enttID, vBackup, vScale));
			}
			ImGui::SameLine(ImGui::GetWindowContentRegionMax().x - fResetBtnWidth);
			if (ImGui::SmallButton("X##Sca"))
			{
				...
				entt::entity enttID = entt::to_entity(Application::Get()->GetCurrentLevel().lock()->GetEntityRegistry(), *this);
				UndoManager::RegisterAction(UndoAction::EntityScale(enttID, vScale, vNew));
				SetLocalScale(vNew);
			}
			ImGui::Custom::Tooltip("Reset");
		}
