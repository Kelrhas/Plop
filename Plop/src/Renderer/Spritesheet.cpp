...
namespace Plop
{
	...
	{
		ASSERTM( _vIndices.x < m_uNbColumn && _vIndices.y < m_uNbRow, "Spritesheet::GetSpriteUV: Wrong coordinates %u %u, spritesheet only defines %u columns and %u rows", _vIndices.x, _vIndices.y, m_uNbColumn, m_uNbRow );
	...
	TextureHandle Spritesheet::GetTextureHandle() const
	{
		return m_hTexture;
	}
