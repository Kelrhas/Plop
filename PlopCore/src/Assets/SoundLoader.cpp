...
namespace Plop::AssetLoader
{
	...
	{
		using result_type = SoundPtr;

		SoundPtr operator()( const StringPath& _path ) const
		{
			SoundPtr xSnd;
	...
	entt::resource_cache<Sound, SoundLoader> s_CacheSound {};

	SoundHandle GetSound( const StringPath& _sFilepath )
	{
		return s_CacheSound.load(entt::hashed_string(_sFilepath.string().c_str()), _sFilepath).first->second;
	}
	...
	SoundHandle PickSoundFromCache()
	{
		...
		if (ImGui::BeginPopupModal( "Pick a loaded sound###PickSoundFromCache", &bOpened ))
		{
			...
			for (auto [id, _hSound] : s_CacheSound)
			{
				if (_hSound)
				{
