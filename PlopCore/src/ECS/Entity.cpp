...
#pragma region CTORS
	...
	Entity::Entity(entt::entity _entityID, entt::registry &_reg) : m_hEntity(_reg, _entityID) {}

	Entity::Entity(entt::handle _hEntity) : m_hEntity(_hEntity) {}

	Entity::Entity(entt::null_t) {}

	Entity::Entity(const Entity &_other) noexcept : m_hEntity(_other.m_hEntity) {}
	...
	{
		if (m_hEntity)
			m_hEntity = entt::handle { *m_hEntity.registry(), static_cast<entt::entity>(entt::null) };
	}
	...
	bool Entity::IsFromLevel(GUID _guidLevel) const
	{
		if (m_hEntity)
		{
			GUID *pGUID = m_hEntity.registry()->ctx().find<GUID>();
...
#pragma region OPERATORS
	...
	bool Entity::operator==(const Entity &_other)
	{
		...
		GUID* guidThis = m_hEntity.registry()->ctx().find<Plop::GUID>();
		GUID *guidOther = _other.m_hEntity.registry()->ctx().find<Plop::GUID>();
	...
	Entity Entity::GetParent() const
	{
		if (m_hEntity)
		{
			...
			if (m_hEntity)
			{
				...
				parent.m_hEntity = entt::handle { *m_hEntity.registry(), graphNode.parent };
			}
...
#pragma region COMPONENTS
	...
	{
		const entt::meta_type type = entt::resolve(_compID);
...
#pragma region SERIALIZATION
	...
	json Entity::ToJson() const
	{
		...
		entt::registry &registry = *m_hEntity.registry();
	...
	{
		ASSERTM(m_hEntity.registry(), "Invalid registry !");
		...
		{
			LevelBase **ppLevel = m_hEntity.registry()->ctx().find<LevelBase *>();
			if (ppLevel && *ppLevel)
			{
