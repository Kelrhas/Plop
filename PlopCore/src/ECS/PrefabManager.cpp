...
- supprimer lien avec un prefab
	...
	*/
	...
	PrefabHandle::operator GUID() const
	{
		return guid;
	}
	...
	bool PrefabManager::CreatePrefab(Entity _entitySrc, const String &_sLibName)
	{
		...
		entity.VisitChildrenRecursive(
		  [&library](Entity _entity)
		  {
	...
	void PrefabManager::UpdatePrefabFromInstance(PrefabHandle _hPrefab, Entity _entity)
	{
		...
		for (auto &[sKey, lib] : s_mapPrefabLibs)
		{
			...
			if (it != lib.vecPrefabs.end())
			{
				...
				const entt::registry &regLevel	   = *entt::handle(_entity).registry();
				...
				for (auto &[prefabEntityGUID, levelEntityGUID] : compInstance.mapping)
				{
					...
					ComponentManager::VisitAllComponents(
					  ...
					  {
						  if (_compId == entt::type_id<Component_Name>().index())
							  return;
						  if (_compId == entt::type_id<Component_GraphNode>().index())
							  return;
						  if (_compId == entt::type_id<Component_PrefabInstance>().index())
							  return;

						  const bool prefabHas = _compInfo.funcHas(regPrefab, prefabEntity);
						  const bool levelHas  = _compInfo.funcHas(regLevel, levelEntity);
		...
		ASSERTM(false, "Prefab not found for entity {}", _entity.GetComponent<Component_Name>().sName.c_str());
	}
	...
	void PrefabManager::DeletePrefab(const Prefab *_pPrefab)
	{
		for (auto &[sKey, lib] : s_mapPrefabLibs)
		{
			...
			if (it != lib.vecPrefabs.end())
			{
				...
				regLevel.view<Component_PrefabInstance>().each(
				  [_pPrefab, &regLevel](entt::entity _enttID, const Component_PrefabInstance &_instanceComp)
				  {
		...
		for (auto &[sKey, lib] : s_mapPrefabLibs)
		{
			...
			if (it != lib.vecPrefabs.end())
			{
				...
				func = [&_jPatch, &prefabInstanceComp, &func](Entity _prefabEntity, Entity _instanceEntity)
				{
					{ // RAII
						json jPrefab;
						ComponentManager::ToJson(_prefabEntity.GetRegistry(), _prefabEntity, jPrefab);
						json jInstance;
						ComponentManager::ToJson(_instanceEntity.GetRegistry(), _instanceEntity, jInstance);
					...
					_prefabEntity.VisitChildren(
					  [&prefabInstanceComp, _instanceEntity, &func](Entity childPrefab)
					  {
						  ...
						  _instanceEntity.VisitChildren(
							[&](Entity _childInstance)
							{
	...
	bool PrefabManager::HasAnyPrefabLib()
	{
		return !s_mapPrefabLibs.empty();
	}
			...
			if (ImGui::BeginChild("PrefabLibs"))
			{
				...
				for (const auto &[sKey, lib] : s_mapPrefabLibs)
				{
					...
					if (ImGui::CollapsingHeader(sKey.c_str(), pOpenBool))
					{
						...
						{
							if (lib.registry.all_of<Component_Name>(prefab.rootId))
							{
				...
				if (ImGui::BeginPopupModal("DeletePrefab", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
				{
					...
					size_t nbInstance = 0;
					regLevel.view<Component_PrefabInstance>().each(
					  [&nbInstance](const Component_PrefabInstance &_instanceComp)
					  {
	...
	void PrefabManager::LoadPrefabLibrary(const StringPath &_sPath)
	{
		...
		VisitAllLibraries(
		  [&](const String &, const PrefabLibrary &lib)
		  {
			  if (lib.sPath == _sPath)
			  {
				  bAlreadyLoaded = true;
