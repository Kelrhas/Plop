...
void SampleLayer::OnUpdate(Plop::TimeStep &_timeStep)
{
	...
	if (xMesh == nullptr)
	{
		...
		float vertices[] = { -0.6f, -0.6f, 0.0f, /*1.f, 0.f, 0.f, 1.f,*/ 0.f, 0.f, -0.6f, 0.6f,	 0.0f, /*1.f, 0.f, 1.f, 1.f,*/ 0.f, 1.f,
							 0.6f,	0.6f,  0.0f, /*0.f, 1.f, 0.f, 1.f,*/ 1.f, 1.f, 0.6f,  -0.6f, 0.0f, /*1.f, 1.f, 0.f, 1.f,*/ 1.f, 0.f };


		xMesh->m_xVertexArray = Plop::VertexArray::Create();
		xMesh->m_xVertexArray->Bind();

		Plop::BufferLayout layout = { { "position", Plop::BufferLayout::ElementType::FLOAT3 },
...
{
	m_xRoot = std::make_shared<UI::Dialog>();

	auto xImg = m_xRoot->CreateElement<UI::Image>("Background");
	xImg.lock()->SetSize(VEC2_1 / 2.f);
	Plop::UI::IUIElement::AddElementTo(xImg, m_xRoot);

	auto xImg2 = m_xRoot->CreateElement<UI::Image>("Second");
	xImg2.lock()->SetSize(VEC2_1 / 2.f);
	Plop::UI::IUIElement::AddElementTo(xImg2, xImg);
}
...
void SampleLayer2D::OnUpdate(Plop::TimeStep &_timeStep)
{
	...
	// m_xRoot->OnUpdate(_timeStep);
}

void SampleLayer2D::OnImGuiRender(Plop::TimeStep &_timeStep)
{
	PROFILING_FUNCTION();

	m_xRoot->OnUpdate(_timeStep);
	m_xRoot->RenderDialog();
}
